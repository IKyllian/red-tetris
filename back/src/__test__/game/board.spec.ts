...
import { ISize, defaultCell, CellType, ICell } from '../../type/cell.interface';
import {
	ITetromino,
	O_TetrominoShape,
	IPosition,
...
describe('board', () => {
	...
	describe('buildBoard', () => {
		it('should return a board object with a size that depends of params with default cells', () => {
			...
			const colums = Array.from({ length: size.columns }, () => ({
				...defaultCell,
			}));
	...
	describe('transferPieceToBoard', () => {
		...
		it('should transfer the piece to the board without fixing it', () => {
			...
			expect(board['cells'][1][1]).toEqual({
				type: CellType.O,
				occupied: false,
			});
			expect(board['cells'][1][2]).toEqual({
				type: CellType.O,
				occupied: false,
			});
			expect(board['cells'][2][1]).toEqual({
				type: CellType.O,
				occupied: false,
			});
			expect(board['cells'][2][2]).toEqual({
				type: CellType.O,
				occupied: false,
			});
		});
	...
	describe('checkForLines', () => {
		...
		it('one line complete, should return 1', () => {
			...
			expect(board['cells'][size.rows - 1]).toEqual(
				expect.arrayContaining(colums)
			);
		});
